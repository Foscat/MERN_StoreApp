import React from 'react';
import { Button, Form, FormGroup, Label, Input } from 'reactstrap';

const ManagerAddProductForm = ({style, name, price, handleFormSubmit, handleInputChange, department, description, quantity, manufacturer}) => {
    return(
        <div className="ManagerForm">
            <Form style={style}>

                <FormGroup>
                    <Label for="productName">Name of product</Label>
                    <Input type="text" name="addProName" defaultValue={name} onChange={handleInputChange}
                     id="productName" placeholder="Product Name" />
                </FormGroup>

                <FormGroup>
                    <Input type="select" name="addProDept" onChange={handleInputChange}
                     id="selectDepartment" multiple defaultValue={department}>
                        <option>General goods</option>
                        <option>Gadgets</option>
                        <option>Clothes</option>
                        <option>Toys</option>
                        <option>Seasonal</option>
                    </Input>
                </FormGroup>

                <FormGroup>
                    <Label for="Manufacturer">Manufacturer</Label>
                    <Input type="text" name="addProManu" id="Manufacturer" defaultValue={manufacturer}
                     onChange={handleInputChange} />
                </FormGroup>

                <FormGroup>
                    <Label for="totalStock">Quantity</Label>
                    <Input type="number" name="addProQuan" id="totalStock" defaultValue={quantity}
                     onChange={handleInputChange} />
                </FormGroup>

                <FormGroup>
                    <Label for="Price">Price</Label>
                    <Input type="number" name="addProPrice" id="Price" defaultValue={price}
                     onChange={handleInputChange} />
                </FormGroup>

                <FormGroup>
                    <Label for="textDescription">Text Area</Label>
                    <Input type="textarea" name="addProDes" id="textDescription" 
                    value={description} onChange={handleInputChange} />
                </FormGroup>

                <Button onClick={handleFormSubmit}>Submit</Button>
            </Form>
        </div>
    )
}

export default ManagerAddProductForm;